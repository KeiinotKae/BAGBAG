...
if (isset($_POST['upload_image']) && isset($_FILES['committee_image'])) {
    $uploadDir = "committee_images/";
    $allowedTypes = ['jpg', 'jpeg', 'png', 'gif'];
    $fileCount = count($_FILES['committee_image']['name']);

    for ($i = 0; $i < $fileCount; $i++) {
        $tmpName = $_FILES['committee_image']['tmp_name'][$i];
        if (!$tmpName) continue;

        $fileName = basename($_FILES['committee_image']['name'][$i]);
        ...
        if (move_uploaded_file($tmpName, $uploadFile)) {
            $stmt = $conn->prepare("INSERT INTO committee_images (image_path) VALUES (?)");
...
if (isset($_GET['delete_image'])) {
    ...
    $stmt = $conn->prepare("SELECT image_path FROM committee_images WHERE id = ?");
    ...
    $stmt = $conn->prepare("DELETE FROM committee_images WHERE id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $stmt->close();
...
// Fetch all images
$sql = "SELECT id, image_path FROM committee_images ORDER BY uploaded_at DESC";
...
<div id="main-content" class="p-8">
    ...
    <form action="" method="post" enctype="multipart/form-data" class="mb-6">
        ...
        <input type="file" name="committee_image[]" multiple accept="image/*" required
