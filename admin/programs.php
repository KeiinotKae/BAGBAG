...
if (isset($_POST['upload_image']) && isset($_FILES['programs'])) {
    $uploadDir = "programs/";
    $allowedTypes = ['jpg', 'jpeg', 'png', 'gif'];
    $fileCount = count($_FILES['programs']['name']);

    for ($i = 0; $i < $fileCount; $i++) {
        $tmpName = $_FILES['programs']['tmp_name'][$i];
        if (!$tmpName) continue;

        $fileName = basename($_FILES['programs']['name'][$i]);
...
// Fetch all images
$sql = "SELECT id, image_path FROM programs ORDER BY uploaded_at DESC";
...
<div id="main-content" class="p-8">
    ...
    <form action="" method="post" enctype="multipart/form-data" class="mb-6">
        ...
        <input type="file" name="programs[]" multiple accept="image/*" required
