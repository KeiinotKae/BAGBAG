...
if (isset($_POST['upload_image']) && isset($_FILES['announcements2'])) {
    $uploadDir = "announcements2/";
    $allowedTypes = ['jpg', 'jpeg', 'png', 'gif'];
    $fileCount = count($_FILES['announcements2']['name']);

    for ($i = 0; $i < $fileCount; $i++) {
        $tmpName = $_FILES['announcements2']['tmp_name'][$i];
        if (!$tmpName) continue;

        $fileName = basename($_FILES['announcements2']['name'][$i]);
        ...
        if (move_uploaded_file($tmpName, $uploadFile)) {
            $stmt = $conn->prepare("INSERT INTO announcements2 (image_path) VALUES (?)");
...
if (isset($_GET['delete_image'])) {
    ...
    $stmt = $conn->prepare("SELECT image_path FROM announcements2 WHERE id = ?");
    ...
    $stmt = $conn->prepare("DELETE FROM announcements2 WHERE id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $stmt->close();
...
// Fetch all images
$sql = "SELECT id, image_path FROM announcements2 ORDER BY uploaded_at DESC";
...
<div id="main-content" class="p-8">
    ...
    <form action="" method="post" enctype="multipart/form-data" class="mb-6">
        ...
        <input type="file" name="announcements2[]" multiple accept="image/*" required
