...
if (isset($_POST['submit_report'])) {
    ...
    if (isset($_FILES['evidence']) && $_FILES['evidence']['error'] === UPLOAD_ERR_OK) {
        ...
        if (($file_type === 'jpg' || $file_type === 'jpeg' || $file_type === 'png' || $file_type === 'gif') && $file_size > $max_image_size) {
            echo "<script>alert('Image size exceeds 5MB limit.'); window.location.href='community_reports.php';</script>";
            exit();
        }

        if (($file_type === 'mp4' || $file_type === 'mov' || $file_type === 'avi') && $file_size > $max_video_size) {
            echo "<script>alert('Video size exceeds 10MB limit.'); window.location.href='community_reports.php';</script>";
            exit();
        }
...
<div id="main-content" class="p-8">
  ...
  <form method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded shadow mb-8">
    <div class="grid grid-cols-2 gap-4">
      ...
      <!-- Evidence Upload -->
      <div class="col-span-2">
        <label class="block text-sm font-medium">Ebidensya (Image or Video, Max: 5MB for Image, 10MB for Video)</label>
